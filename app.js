...
// تحميل قائمة الحلاقين مع فلترة حسب المدينة
async function loadBarbers() {
    ...
    onValue(ref(database, 'barbers'), (snapshot) => {
        ...
        // تحديث قائمة المدن في الفلتر
        const cities = new Set();
        Object.values(barbers).forEach(barber => {
            if (barber.city) cities.add(barber.city);
        });
        ...
        // فلترة الحلاقين حسب المدينة المختارة
        const selectedCity = cityFilter.value;
        const filteredBarbers = Object.entries(barbers).filter(([id, barber]) => {
            return !selectedCity || barber.city === selectedCity;
        });
        
        if (filteredBarbers.length === 0) {
            barbersList.innerHTML = '<div class="alert alert-info">لا يوجد حلاقون متاحون في المدينة المحددة</div>';
            return;
        }
        
        filteredBarbers.forEach(([id, barber]) => {
    ...
    // إضافة حدث تغيير للفلتر
    cityFilter.addEventListener('change', loadBarbers);
}
...
async function bookAppointment(barberId, barberName) {
    ...
    try {
        ...
        // جلب بيانات الحلاق لمعرفة المدينة
...
function showCurrentBooking() {
    ...
    document.getElementById('bookingCity').textContent = currentUser.booking.barberCity;
